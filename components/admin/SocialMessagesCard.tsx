...
export function SocialMessagesCard({ businessId }: SocialMessagesCardProps) {
  ...
  const getPlatformColor = (platform: string) => {
    const colors: Record<string, string> = {
      telegram: 'bg-blue-500/20 text-blue-400 border-blue-500/50',
      instagram: 'bg-pink-500/20 text-pink-400 border-pink-500/50',
      whatsapp: 'bg-green-500/20 text-green-400 border-green-500/50',
      facebook: 'bg-blue-600/20 text-blue-500 border-blue-600/50',
      viber: 'bg-purple-500/20 text-purple-400 border-purple-500/50',
    }
  ...
  return (
    <div className="rounded-xl p-6 card-floating">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-white mb-1" style={{ letterSpacing: '-0.01em' }}>
            Повідомлення
          </h3>
          <p className="text-xs text-gray-400">З соціальних мереж</p>
        </div>
        {unreadCount > 0 && (
          <div className="px-2 py-1 bg-red-500 rounded-full text-xs font-bold text-white min-w-[20px] text-center">
            {unreadCount}
          </div>
        )}
      </div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto mb-4">
            {messages.map((message) => (
              <button
                ...
                className={cn(
                  'w-full text-left bg-white/5 border rounded-lg p-3 transition-colors',
                  !message.isRead
                  ...
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      {selectedMessage.avatar ? (
                        <img src={selectedMessage.avatar} alt={selectedMessage.senderName} className="w-full h-full rounded-full object-cover" />
                      ) : (
                        <div className="text-white">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400">{getPlatformName(selectedMessage.platform)}</div>
                    </div>
                  </div>
                  <button
