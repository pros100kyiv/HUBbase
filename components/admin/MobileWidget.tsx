...
export function MobileWidget({
  ...
  const baseClasses = 'rounded-candy-sm bg-gray-800 dark:bg-gray-900 border border-gray-700 backdrop-blur-sm p-3 shadow-soft-xl transition-all duration-200 active:scale-[0.97] overflow-hidden'
      ...
      >
        ...
        {title && (
          <div className="mb-1.5">
            <p className="text-xs text-gray-300 dark:text-gray-300 font-bold uppercase tracking-wider mb-0.5">
              {title}
            </p>
            {subtitle && (
              <p className="text-[10px] text-gray-400 dark:text-gray-400 leading-tight">{subtitle}</p>
            )}
          </div>
        )}
        {value !== undefined && (
