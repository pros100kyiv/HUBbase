  ...
  return (
    <div className="rounded-xl p-4 md:p-6 card-floating">
      <div className="flex items-center justify-between mb-3 md:mb-4">
        ...
        <button className="text-xs md:text-sm text-gray-300 hover:text-white transition-colors hidden md:block">
          Open Archive &gt;
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-3">
        {displayTasks.map((task) => (
          <div 
            key={task.id}
            className="bg-white/5 rounded-lg p-3 md:p-4 relative border border-white/10"
          >
            <div className="flex items-start justify-between mb-2 md:mb-3">
              <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0">
                {task.icon === 'users' ? (
                  <UsersIcon className="w-4 h-4 md:w-5 md:h-5 text-gray-300" />
                ) : (
            ...
            {openMenuId === task.id && (
              <div className="absolute top-10 md:top-12 right-2 bg-[#2A2A2A] border border-white/20 rounded-lg shadow-lg z-10 min-w-[120px]">
                <button
                  onClick={() => {
                    onAddNote?.(task.id)
                    setOpenMenuId(null)
            ...
            <h4 className="text-xs md:text-sm font-semibold text-white mb-0.5 md:mb-1 line-clamp-2">{task.title}</h4>
