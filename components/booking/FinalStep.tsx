...
export function FinalStep({ businessId }: FinalStepProps) {
  const { state, setClientName, setClientPhone, setStep, setConfirmation } = useBooking()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const appointmentId = typeof created?.id === 'string' ? created.id : 'unknown'
        const createdAt = typeof created?.createdAt === 'string' ? created.createdAt : new Date().toISOString()
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="rounded-xl p-3 sm:p-4 mb-4 card-glass">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Ім'я *</label>
              <input
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Телефон *</label>
              <input
        ...
        <div className="rounded-xl p-4 mb-4 card-glass">
          <h3 className="text-base font-semibold mb-3 text-foreground">Деталі запису:</h3>
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Спеціаліст:</span><span className="font-medium text-foreground dark:text-white">{state.selectedMaster?.name}</span></div>
            <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Дата та час:</span><span className="font-medium text-foreground dark:text-white">{state.selectedDate && format(state.selectedDate, 'd MMMM yyyy', { locale: uk })}, {state.selectedTime}</span></div>
            <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Послуги:</span><span className="font-medium text-foreground dark:text-white text-right max-w-[60%]">
            ...
            <div className="flex justify-between pt-2 border-t border-black/10 dark:border-white/10"><span className="font-semibold text-foreground dark:text-white">Всього:</span><span className="text-lg font-bold text-purple-400">{isPriceAfterProcedure ? 'Вартість узгоджується після процедури' : `${Math.round(totalPrice)} ₴`}</span></div>
          </div>
        </div>

        <div className="flex gap-2 sm:gap-3">
          <button type="button" onClick={() => setStep(3)} className="touch-target flex-1 min-h-[48px] py-2.5 rounded-lg border border-black/10 dark:border-white/20 bg-black/[0.04] dark:bg-white/10 text-foreground dark:text-white text-sm font-medium hover:bg-black/[0.06] dark:hover:bg-white/20 transition-colors active:scale-[0.98]">Назад</button>
          <button
            type="button"
